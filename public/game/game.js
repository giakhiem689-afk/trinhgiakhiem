...
function drawTile(ctx, tile, tx, ty) {
    ...
    switch (tile) {
        ...
        case 'w': // Low Invisible Wall
            if (player && player.ninjaSightActiveTimer > 0) {
                ctx.fillStyle = 'rgba(56, 189, 248, 0.25)';
                ctx.fillRect(0, 0, 25, 25);
                ctx.strokeStyle = '#00f3ff';
                ctx.lineWidth = 2;
                ctx.strokeRect(1, 1, 23, 23);
                ctx.fillStyle = '#00f3ff';
                ctx.fillRect(0, 0, 25, 3); // top glowing bar
            }
            break;

        case 'W': // High Invisible Wall
            if (player && player.ninjaSightActiveTimer > 0) {
                ...
                ctx.lineWidth = 2;
                ctx.strokeRect(1, 1, 23, 23);
                // Down arrow symbol telling player to crouch!
                ctx.fillStyle = '#e9d5ff';
                ctx.beginPath();
                ctx.moveTo(7, 8); ctx.lineTo(18, 8); ctx.lineTo(12.5, 17);
                ctx.closePath();
                ctx.fill();
            }
            break;

        case 'x': // Invisible Cactus
            if (player && player.ninjaSightActiveTimer > 0) {
                ...
                ctx.lineWidth = 2;
                ctx.strokeRect(1, 1, 23, 23);
                // Draw red cactus outline
                ctx.fillStyle = '#dc2626';
                ctx.fillRect(9, 4, 7, 21); // body
                ctx.fillRect(3, 10, 6, 4);  // left arm
                ctx.fillRect(3, 6, 4, 8);
                ctx.fillRect(16, 12, 6, 4); // right arm
                ctx.fillRect(18, 8, 4, 8);
            }
            break;

        case 'g': // Invisible Wind Column
            if (player && player.ninjaSightActiveTimer > 0) {
                ...
                ctx.lineWidth = 1.5;
                ctx.beginPath();
                const windOffset = (Date.now() * 0.02) % 25;
                ctx.moveTo(4, 25 - windOffset); ctx.lineTo(12, 12 - windOffset); ctx.lineTo(20, 25 - windOffset);
                ctx.stroke();
            }
            break;
    }
...
function triggerScreenShake(duration = 0.3, intensity = 8) {
...
function updateHUD() {
    ...
    const sightElem = document.getElementById('sightVal');
    if (sightElem) {
        if (player.ninjaSightActiveTimer > 0) {
            sightElem.innerText = `👁️ ACTIVE (${player.ninjaSightActiveTimer.toFixed(1)}s)`;
            sightElem.className = 'hud-value sight-active';
        } else if (player.ninjaSightCooldown > 0) {
            sightElem.innerText = `⏳ (${player.ninjaSightCooldown.toFixed(0)}s)`;
            sightElem.className = 'hud-value sight-cd';
        } else {
            sightElem.innerText = `👁️ READY [E]`;
            sightElem.className = 'hud-value sight-ready';
        }
    }
}
...
window.addEventListener('keydown', e => {
    ...
    if (['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'Space', ' ', 'KeyW', 'KeyS', 'KeyA', 'KeyD', 'KeyE', 'KeyC', 'KeyR'].includes(e.code) || e.keyCode === 32) {
        e.preventDefault();
    }
    ...
    if ((e.key === 'e' || e.key === 'E' || e.key === 'c' || e.key === 'C') && player) {
        player.toggleNinjaSight(Sound);
        return;
    }
...
function setupMobileControls() {
    ...
    if (eyeBtn) {
        const handleEye = (e) => {
            e.preventDefault();
            Sound.init();
            if (player) player.toggleNinjaSight(Sound);
        };
        eyeBtn.addEventListener('touchstart', handleEye, { passive: false });
        eyeBtn.addEventListener('mousedown', handleEye);
    }
