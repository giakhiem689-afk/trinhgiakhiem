    ...
    playSFX(type) {
        ...
        switch (type) {
            ...
            case 'stage_clear':
                ...
                break;
            case 'wind':
                // Whoosh wind gust sound
                this.synthesizeSweep(now, 150, 450, 0.25, 'sine', 0.5);
                this.synthesizeSweep(now + 0.05, 300, 100, 0.2, 'triangle', 0.4);
                break;
            case 'wall_hit':
                // Thud / glass clink
                this.synthesizeSweep(now, 600, 150, 0.08, 'sawtooth', 0.6);
                this.synthesizeTone(now, 1200, 0.04, 'sine');
                break;
            case 'cactus_sting':
                // Prick / sharp sting sound
                this.synthesizeTone(now, 1400, 0.04, 'sawtooth');
                this.synthesizeSweep(now + 0.03, 1000, 200, 0.1, 'sawtooth', 0.8);
                break;
            case 'ninja_sight':
                // Mystic chakra resonance / Byakugan activation sound
                this.synthesizeSweep(now, 300, 1500, 0.2, 'sine', 0.7);
                this.synthesizeTone(now + 0.15, 1760, 0.25, 'sine');
                break;
            case 'near_miss':
                // Thrilling ding & harp slide
                this.synthesizeTone(now, 1046.50, 0.08, 'sine'); // C6
                this.synthesizeTone(now + 0.06, 1567.98, 0.12, 'sine'); // G6
                break;
            case 'combo':
                // Escalating cheer chime
                const comboFreqs = [523.25, 659.25, 783.99, 1046.50];
                comboFreqs.forEach((f, idx) => {
                    this.synthesizeTone(now + idx * 0.04, f, 0.08, 'triangle');
                });
                break;
        }
    }
