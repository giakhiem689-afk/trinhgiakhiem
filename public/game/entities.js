    ...
    handleTileCollision(level, isHorizontal) {
        ...
        for (let row = startY; row <= endY; row++) {
            for (let col = startX; col <= endX; col++) {
                ...
                // Non-solid tiles ignored by basic physics engine
                if (tile && tile !== '.' && tile !== 'C' && tile !== 'F' && tile !== 'f' && tile !== 'h' && tile !== 'd' && tile !== 'g' && tile !== 'x') {
                    
                    // High invisible wall 'W' can be passed under if player is crouching!
                    if (tile === 'W' && this.type === 'player' && this.isCrouching) {
                        continue;
                    }
...
class Player extends Entity {
    constructor(x, y) {
        ...
        // Ninja Sight & Trap FX properties
        this.ninjaSightActiveTimer = 0;
        this.ninjaSightCooldown = 0;
        this.comboStreak = 0;
        this.comboTimer = 0;
    ...
    toggleNinjaSight(sound) {
        if (this.ninjaSightCooldown <= 0) {
            this.ninjaSightActiveTimer = 1.5; // Active for 1.5 seconds
            this.ninjaSightCooldown = 8.0;   // 8 second cooldown
            if (sound) sound.playSFX('ninja_sight');
            return true;
        }
        return false;
    }
    ...
    update(dt, keys, level, sound, spawnParticle, spawnItem) {
        ...
        // Decrement timers
        if (this.ninjaSightActiveTimer > 0) this.ninjaSightActiveTimer -= dt;
        if (this.ninjaSightCooldown > 0) this.ninjaSightCooldown -= dt;
        ...
        if (keys.jump && this.isGrounded) {
            ...
            this.isGrounded = false;
            sound.playSFX('jump');
            spawnParticle(new DustParticle(this.x + this.width / 2, this.y + this.height));
        }
        ...
        // Crouch (Allowed for both small and big Naruto to chui qua W wall!)
        if (keys.down && this.isGrounded) {
            if (!this.isCrouching) {
                this.isCrouching = true;
                this.height = this.state === 0 ? 18 : 26;
            }
            this.vx *= 0.82;
        } else {
            if (this.isCrouching) {
                this.isCrouching = false;
                const normalH = this.state === 0 ? 28 : 46;
                this.y -= (normalH - this.height);
                this.height = normalH;
            }
        }
        ...
        const speedBeforeFall = this.vy;

        super.update(dt, level);

        // Landing shockwave & dust
        if (!wasGrounded && this.isGrounded && speedBeforeFall > 300) {
            spawnParticle(new DustParticle(this.x + 2, this.y + this.height));
            spawnParticle(new DustParticle(this.x + this.width - 2, this.y + this.height));
        }
        ...
        if (this.vx !== 0 && this.isGrounded) {
            ...
            if (this.dustSpawnTimer > 0.12 && Math.abs(this.vx) > 100) {
                spawnParticle(new DustParticle(this.x + (this.direction === 1 ? 0 : this.width), this.y + this.height));
                this.dustSpawnTimer = 0;
            }
        } else {
            this.walkFrame = 0;
        ...
        for (let r = startY; r <= endY; r++) {
            for (let c = startX; c <= endX; c++) {
                if (r >= level.height || c >= level.width) continue;
                const tile = level.map[r][c];

                if (tile === 'C') { // Ramen collect
                    level.map[r] = level.map[r].substring(0, c) + '.' + level.map[r].substring(c + 1);
                    this.coins++;
                    this.score += 200;
                    this.registerCombo(sound, spawnParticle, c * tileSize + 12, r * tileSize, 200);
                    sound.playSFX('coin');
                } else if (tile === 'x' && !this.isInvincible && !this.isDying) {
                    // Invisible Cactus hit!
                    spawnParticle(new CactusNeedleParticle(this.x + this.width / 2, this.y + this.height / 2));
                    if (typeof triggerScreenShake === 'function') triggerScreenShake(0.35, 10);
                    sound.playSFX('cactus_sting');
                    if (this.state > 0) {
                        this.shrink(sound);
                    } else {
                        this.die(sound);
                    ...
                    // Invisible Wind Column!
                    const windStrength = c > 70 ? 420 : -420;
                    this.vx += windStrength * dt;
                    if (Math.random() < 0.35) {
                        spawnParticle(new WindParticle(this.x + (Math.random() - 0.5) * 40, this.y + Math.random() * this.height, windStrength > 0 ? 300 : -300));
                    }
                    this.windSFXTimer += dt;
                    if (this.windSFXTimer > 0.45) {
                        sound.playSFX('wind');
                        this.windSFXTimer = 0;
                    }
                }
        ...
        // --- NEAR-MISS DETECTION MECHANIC ---
        if (!this.isGrounded && this.vy > 0 && this.nearMissCooldown <= 0) {
            // Check if player is closely leaping above an invisible cactus 'x' or a pit
            const belowTileY = Math.floor((this.y + this.height + 6) / tileSize);
            const centerCol = Math.floor((this.x + this.width / 2) / tileSize);
            if (belowTileY < level.height && centerCol >= 0 && centerCol < level.width) {
                const tileBelow = level.map[belowTileY][centerCol];
                if (tileBelow === 'x' || tileBelow === '.') {
                    this.nearMissCooldown = 1.2;
                    this.score += 300;
                    sound.playSFX('near_miss');
                    spawnParticle(new ScoreText(this.x + 5, this.y - 12, "NEAR MISS! +300"));
                }
            }
        }
    ...
    registerCombo(sound, spawnParticle, x, y, baseScore) {
        this.comboStreak++;
        this.comboTimer = 2.5;
        const multiplier = Math.min(4, Math.floor(this.comboStreak / 2) + 1);
        const finalScore = baseScore * multiplier;
        
        if (this.comboStreak > 1) {
            sound.playSFX('combo');
            spawnParticle(new ComboPopupText(x, y - 10, `STREAK x${this.comboStreak}! +${finalScore}`));
        } else {
            spawnParticle(new ScoreText(x, y, `+${finalScore}`));
        }
    }

    onWallCollide(tile) {
        if (tile === 'w' || tile === 'W') {
            if (typeof Sound !== 'undefined') Sound.playSFX('wall_hit');
            if (typeof particles !== 'undefined') {
                particles.push(new RippleParticle(this.x + (this.direction === 1 ? this.width : 0), this.y + this.height / 2));
            }
        }
    }
    ...
    draw(ctx, cameraX) {
        ...
        // --- POWERFUL CHAKRA AURA FX ---
        const timePulse = (Date.now() * 0.006) % (Math.PI * 2);
        const glowPulse = Math.sin(timePulse) * 4;
        ctx.save();
        if (this.state === 0) {
            // Blue Cyan Chakra Spirit Aura
            ...
            // Golden Bijuu Kurama Chakra Aura & Lightning Sparks
            ...
            // Crackling Yellow Lightning Arcs
            ctx.strokeStyle = '#fef08a';
            ctx.lineWidth = 1.5;
            ctx.beginPath();
            ctx.moveTo(w / 2, -4);
            ctx.lineTo(w / 2 + (Math.random() - 0.5) * 18, -12);
            ctx.lineTo(w / 2 + (Math.random() - 0.5) * 18, -20);
            ctx.stroke();
        }
        ctx.restore();
...
class DustParticle {
    ...
    draw(ctx, cameraX) {
        ctx.save();
        ctx.translate(this.x - cameraX, this.y);
        ctx.fillStyle = `rgba(226, 232, 240, ${this.life / this.maxLife})`;
        ctx.beginPath();
...
class RippleParticle {
    ...
    draw(ctx, cameraX) {
        ctx.save();
        ctx.translate(this.x - cameraX, this.y);
        ctx.strokeStyle = `rgba(56, 189, 248, ${this.life / this.maxLife})`;
        ctx.lineWidth = 2;
        ctx.beginPath();
...
class WindParticle {
    constructor(x, y, windVx) {
        this.x = x;
        this.y = y;
        this.vx = windVx + (Math.random() - 0.5) * 50;
    ...
    draw(ctx, cameraX) {
        ctx.save();
        ctx.translate(this.x - cameraX, this.y);
        ctx.strokeStyle = `rgba(148, 163, 184, ${this.life / this.maxLife * 0.7})`;
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.moveTo(0, 0);
        ctx.lineTo(this.vx > 0 ? -this.length : this.length, 0);
        ctx.stroke();
        ctx.restore();
    }
}

class CactusNeedleParticle {
    constructor(x, y) {
        this.x = x;
        this.y = y;
        this.sparks = [];
    ...
    draw(ctx, cameraX) {
        ...
        this.sparks.forEach(s => {
            ctx.fillRect(s.vx * progress * 0.4, s.vy * progress * 0.4, 3, 3);
        });
        ctx.restore();
    }
}

class ComboPopupText {
    constructor(x, y, text) {
        this.x = x;
        this.y = y;
        this.vy = -75;
